/**
 * One-shot Magmos pack: thriller slide film + UGC vertical from Venice stills/VO.
 * Run: npx tsx scripts/magmos-see-pack.ts
 */
import { existsSync, mkdirSync, writeFileSync, copyFileSync } from "node:fs";
import { join } from "node:path";
import { DATA_DIR, assertDataDir } from "../src/config.js";
import { hasFfmpeg, runFfmpeg } from "../src/edit/ffmpeg-util.js";
import { veniceGenerateImage, veniceTextToSpeech, hasVenice } from "../src/integrations/venice.js";
import { scaffoldFromTrailer, renderHyperframes } from "../src/integrations/hyperframes.js";
import { produceTrailer } from "../src/studio/trailer.js";
import { generateDraft, formatDraftForCopy } from "../src/generate/draft.js";
import { generateCreative, formatCreative } from "../src/teams/creative.js";
...
console.log("[2] Thriller poster + UGC influencer still + VOs");
...
if (hasFfmpeg()) {
  ...
  const slides = [
    ...
    ...ads.slice(0, 3).map((p) => join(packDir, p.split(/[/\\]/).pop()!)),
  ];
...
const md = [
  ...
  "",
  "## Open these",
  thrillerMp4 && existsSync(thrillerMp4)
    ? `- Thriller video: \`${thrillerMp4}\``
    : "- Thriller video: (ffmpeg failed — open stills)",
  ugcMp4 && existsSync(ugcMp4)
    ? `- UGC influencer video: \`${ugcMp4}\``
    : "- UGC influencer video: open `ugc-influencer-pov.png` + `ugc-influencer-vo.mp3`",
  ...
  "",
  "## Note",
  "Venice T2V queue timed out — these clips use stills + VO so you can review now.",
