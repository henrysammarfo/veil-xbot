...
export function buildPaidGrowthPack(projectId: string): PaidGrowthPack {
  ...
  const sections: string[] = [
    ...
    ``,
    `**Focus:** Magmos dollar ads + verified account + promoted distribution.`,
    ...
    `- [ ] Pin Magmos ad video after first promoted post`,
  ...
  if (projectId === "magmos") {
    sections.push(
      ...
      `\`\`\`bash`,
      `npm run edit-auto demo-recording.mp4 magmos`,
      `npm run magmos-ad demo-recording.mp4`,
      `\`\`\``,
      ``,
      `Style: **${adStyle.id}** — beat-sync, music bed, captions, b-roll, SFX`,
      `Hook on screen: **Still $1. Can earn while you hold it.**`,
      ``,
      ...
      `- Hook: product UI on phone — plain dollar story, no jargon`,
      ``,
